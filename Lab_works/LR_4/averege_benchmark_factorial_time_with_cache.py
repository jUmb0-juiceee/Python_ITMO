...
def benchmark(func, n: int, number=1, repeat=5) -> int:
    """Возвращает среднее время выполнения func для числа n"""
    # производим несколько повторов и возвращаем усреднённое время
...
def main():
    """
    Основная функция программы, в которой формируется набор данных, происходит вызов бэнчмарка по среднему времени
    ...
    for n in test_data:
        res_recursive.append(benchmark(fact_recursive, n, number=1000, repeat=4))
        res_iterative.append(benchmark(fact_iterative, n, number=1000, repeat=4))
